...
class RichCardsBot {
    ...
    async onTurn(turnContext) {
        if (turnContext.activity.type === ActivityTypes.Message) {
            console.log("turn context text",turnContext.activity.text);
            ...
            if (!turnContext.responded && results.status === DialogTurnStatus.empty) {
                await turnContext.sendActivity('Welcome to United Employee Services !!!');
                // Create the PromptOptions which contain the prompt and reprompt messages.
                // PromptOptions also contains the list of choices available to the user.
                const promptOptions = {
                    prompt: '',
                    reprompt: 'That was not a valid choice, try again.',
            ...
            // The bot parsed a valid response from user's prompt response and so it must respond.
            ...
            else if (turnContext.activity.text == 'Flight Reservation') {
                await turnContext.sendActivity(`What would you like to do in Flight Reservation`);
               const promptOptions2 = {
                ...
                // Prompt the user with the configured PromptOptions.
                await dc.prompt('cardPrompt', promptOptions2);
            }
    ...
    async sendCardResponse(turnContext, dialogTurnResult) {
        switch (dialogTurnResult.result.value) {
            case 'Check Policy':
                turnContext.sendActivity(`Sorry I am still learning and would request you to approach the HR for policy details`);
            ...
            case 'Flight Reservation':
               //await turnContext.sendActivity(`What would you like to do in Flight Reservation`);
                ...
                break;
            case 'Leaves':
                turnContext.sendActivity(`Sorry leave application feature is yet not rolled out. Thank you for trying`);
           ...
           /* case 'Receipt Card':
            ...
            case 'All Cards':
                ...
                break;*/
            default:
                await turnContext.sendActivity('I guess I did not understand or this feature is not yet available.');
        }
    }
    ...
    getChoices() {
        const cardOptions = [
            {
                value: 'Check Policy',
                synonyms: ['1', 'check policy', 'Check Policy','CheckPolicy']
            },
            {
                value: 'Flight Reservation',
                synonyms: ['2', 'Flight Reservation', 'flight reservation','FlightReservation','check my flight reservation','Check my flight resevation']
            },
            {
                value: 'Leaves',
                synonyms: ['3', 'Leaves', 'leaves','Apply leaves','I want to apply leaves']
            }/*,
            {
                value: 'Receipt Card',
     ...
     getChoices2() {
        const cardOptions2 = [
            {
                value: 'Wishlist',
                synonyms: ['1', 'Wishlist', 'Wish list','wish list']
            },
            {
                value: 'Booking',
                synonyms: ['2', 'Booking', 'booking','book flight','book me a flight']
            },
            {
                value: 'Passes',
                synonyms: ['3', 'Passes', 'passes','check my passes']
            },
            {
                value: 'Seat Availability',
                synonyms: ['4', 'Seat Availability', 'seat availability','check seat availability']
            }/*,
            {
                value: 'Signin Card',
                synonyms: ['5', 'signin', 'signin card']
